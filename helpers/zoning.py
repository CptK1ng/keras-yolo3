...
class ZoneController:
    ...
    def addZone(self, image, type):
        ...
        pd = PolygonDrawer("Polygon", self.drawZones(image.copy()))
        pd.run()
    ...
    def getTouchingZonesForObject(self, bbox):
...
def test_zoning():
    ...
    z = ZoneController()
    z.addZone(image, type=0)
    z.addZone(image, type=1)

    imageWithZones = z.drawZones(image)
    cv2.imshow("test", imageWithZones)
    cv2.waitKey()

    #foreach person bbox:
    touching = z.getTouchingZonesForObject((100,100,200,200)) #xmin, ymin, xmax, ymax
    if len(touching) > 0:
